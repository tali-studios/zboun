...
export async function updateRestaurantSettingsAction(
  ...
  // Handle regular delivery tiers - only process when free delivery is OFF
  // When free delivery is ON, preserve existing tiers so they're not lost when toggling
  if (!freeDelivery) {
  ...
  // Handle fast delivery tiers - only process when fast delivery is enabled
  // When fast delivery is OFF, preserve existing tiers so they're not lost when toggling
  if (fastDeliveryEnabled) {
    // Delete existing fast tiers first
